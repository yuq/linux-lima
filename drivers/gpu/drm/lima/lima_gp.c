...
static irqreturn_t lima_gp_irq_handler(int irq, void *data)
{
	...
	if (state & LIMA_GP_IRQ_MASK_ERROR) {
		if (state & LIMA_GP_IRQ_PLBU_OUT_OF_MEM)
			dev_info(dev->dev, "gp out of heap irq state=%x status=%x\n",
				 state, status);
		else {
			     ...
			     struct lima_sched_task *task)
{
	...
	for (i = 0; i < task->num_bos; i++) {
		...
		if (bo->heap_size &&
		    lima_vm_get_va(task->vm, bo) == f[LIMA_GP_PLBU_ALLOC_START_ADDR]) {
			f[LIMA_GP_PLBU_ALLOC_END_ADDR] =
				f[LIMA_GP_PLBU_ALLOC_START_ADDR] + bo->heap_size;
...
static int lima_gp_task_recover(struct lima_sched_pipe *pipe)
{
	...
	struct drm_lima_gp_frame *frame = task->frame;
	u32 *f = frame->frame;
	size_t fail_size = f[LIMA_GP_PLBU_ALLOC_END_ADDR] - f[LIMA_GP_PLBU_ALLOC_START_ADDR];
	...
	gp_write(LIMA_GP_PLBU_ALLOC_END_ADDR,
		 f[LIMA_GP_PLBU_ALLOC_START_ADDR] + task->heap->heap_size);
	gp_write(LIMA_GP_CMD, LIMA_GP_CMD_UPDATE_PLBU_ALLOC);
	return 0;
}
